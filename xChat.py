...
# Globals
DEFAULT_MESSAGE = "61-70 FGW TASI -- 399K -- YÜZÜKCÜ ÖNÜ -- X: 103, Y: 15"
MESSAGES_DELAY = 60000 # delay between messages (ms)
...
cbxLogUnknown = QtBind.createCheckBox(gui,'cbxLog_clicked','All Others',21, 178)

lblMessageDelay = QtBind.createLabel(gui,"Message Interval (ms):",21,222)
tbxMessageDelay = QtBind.createLineEdit(gui,"",141, 220, 200, 18)
lblLog = QtBind.createLabel(gui,"Silkroad does not allow you to message in a short time of interval. Therefore make sure you put enough interval to be not blocked by the game.",21,240)

cbxLogStall = QtBind.createCheckBox(gui,'cbxLog_clicked','Stall',221,64)
cbxLogGlobal = QtBind.createCheckBox(gui,'cbxLog_clicked','Global',221,83)
cbxLogNotice = QtBind.createCheckBox(gui,'cbxLog_clicked','Notice',221,102)
cbxLogGM = QtBind.createCheckBox(gui,'cbxLog_clicked','GM',221,121)
cbxEvtDrop_item = QtBind.createCheckBox(gui,'cbxLog_clicked','Item drop',221,140)
cbxEvtDrop_rare = QtBind.createCheckBox(gui,'cbxLog_clicked','Rare drop',221,159)

cbxEvtSpawn_unique = QtBind.createCheckBox(gui,'cbxLog_clicked','Unique spawn',421, 64)
cbxEvtSpawn_hunter = QtBind.createCheckBox(gui,'cbxLog_clicked','Hunter/Trader spawn',421,83)
cbxEvtSpawn_thief = QtBind.createCheckBox(gui,'cbxLog_clicked','Thief spawn',421,102)
cbxEvtChar_attacked = QtBind.createCheckBox(gui,'cbxLog_clicked','Character attacked',421,121)
cbxEvtChar_died = QtBind.createCheckBox(gui,'cbxLog_clicked','Character died',421,140)
cbxEvtPet_transport_died = QtBind.createCheckBox(gui,'cbxLog_clicked','Transport/Horse died',421,159)
...
def cbxMsg_clicked(checked):
	...
	if checked:
		...
		delay = determined_delay if determined_delay != "" else str(MESSAGES_DELAY)
		log("Plugin: Starting spam with interval " + delay + "ms.")
	else:
...
def event_loop():
	if character_data and cbxMsg_checked:
		...
		determined_delay = QtBind.text(gui,tbxMessageDelay)
		DELAY = int(determined_delay) if determined_delay != "" else MESSAGES_DELAY
